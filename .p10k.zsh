# Generated by Powerlevel10k configuration wizard on 2025-12-23 at 15:14 -05.
...
() {
  ...
  # No separator after os_icon.
  typeset -g POWERLEVEL9K_OS_ICON_LEFT_SEGMENT_SEPARATOR=''
  # No whitespace after os_icon.
  ...
  # Display directory name in bold.
  ...
  # Custom AWS profile segment
  function prompt_my_aws() {
    # Get the current AWS Account ID from the active session
    local current_account_id=$(aws sts get-caller-identity 2>/dev/null | grep -o '"Account": "[^"]*"' | cut -d'"' -f4)
    
    # If no active session, don't show anything
    [[ -z "$current_account_id" ]] && return
    
    # Read ~/.aws/config and find the profile that matches the account ID
    local aws_profile=""
    ...
    while IFS= read -r line; do
      if [[ "$line" =~ '^\[profile (.+)\]' ]]; then
        ...
        in_profile=true
      elif [[ "$line" =~ '^sso_account_id *= *(.+)$' ]] && [[ "$in_profile" == true ]]; then
        local account_id="${match[1]}"
    ...
    # Only show if we found a matching profile
    if [[ -n "$aws_profile" ]]; then
      p10k segment -f 226 -t "$aws_profile"
    fi
  }

  function instant_prompt_my_aws() {
    prompt_my_aws
  }
